...
ANSWER: The data set is called iris and the set is proving the length of width of sepal and petal of three different flowers in columns and the three species flowers are setosa, versicolor and virginica.
...
```{r}
 ...
 f <- c(5,6,7,2,5,1,3,6,2,5,1,1,4,1,5,3,8,1,0,4,3)
...
#'
#' TO-DO: assumimg the data is linearly separable, it finds the the separator for the data 
#'
#' @param Xinput      TO-DO: n x p matrx
#' @param y_binary    TO-DO:  gives binary outpur (0,1)
#' @param MAX_ITER    TO-DO: it is telling how many time it will run and its 1000
#' @param w           TO-DO: w belongs to R^ (P+1)
#'
...
TO-DO: Ans:  here we have two feature: X is our first feature and y is our 2nd. the graph is returning to O and 1 which we got it from the data and 0 , 1 represent by different colors 
...
Explain this picture. Why is this line of separation not "satisfying" to you?
 ...
 It didnt seperate the data perfectly as it covored one red dot which is 0.
...
Is this the same as what the `e1071` implementation returned? Why or why not? no.

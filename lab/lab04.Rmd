...
p is 1 and n is 928 the number of observations. here its giving us data about the  parents and child and it showing the how child's height is deperndent on parents height
...
b_1 is showing the childs height how the childs height change by their parents and it is positively related which is .646.
RMSE is the variations of child's height and it is 2.238.
R^2 is .21 which means 21.05% variation is explained here by the independent variables.


How good is this model? How well does it predict? Discuss.
the model is not good model as the value of R^2 is pretty low so it does not predict very weel.
...
It is reasonable as child gets the genes from their parents.

If they were to have the same height and any differences were just random noise with expectation 0, what would the values of $\beta_0$ and $\beta_1$ be?

the beta_0 and beta_1 is 0.
...
because the height of the childs are closest to the mean 

Why should this effect be real?

this effect should be real because it shows how the height of the childern of same parents drew nearer to the mean after the first.

You now have unlocked the mystery. Why is it that when modeling with $y$ continuous, everyone calls it "regression"? Write a better, more descriptive and appropriate name for building predictive models with $y$ continuous.

it is called regression because the more sample is added, it regressed towards mean. it should be continuous regression model 
